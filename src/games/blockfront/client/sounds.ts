...
export function defineSounds(client: Client) {
  ...
  a.define('blaster_rifle_rebels', (s) => bolt(s, { from: 2600, to: 340, len: 0.15, snap: 0.3, ring: 0.08, loud: 1.4 }));
  a.define('blaster_rifle_empire', (s) => bolt(s, { from: 2050, to: 250, len: 0.14, snap: 0.45, ring: 0.04, grit: 0.08, loud: 1.4 }));
  a.define('blaster_heavy_rebels', (s) => bolt(s, { from: 2100, to: 300, len: 0.1, snap: 0.25, loud: 1.2, ring: 0.05 }));
  a.define('blaster_heavy_empire', (s) => bolt(s, { from: 1700, to: 230, len: 0.1, snap: 0.35, loud: 1.2, ring: 0.03, grit: 0.1 }));
  ...
  a.define('blaster_pistol_rebels', (s) => bolt(s, { from: 2900, to: 430, len: 0.12, snap: 0.4, loud: 1.35, ring: 0.07 }));
  a.define('blaster_pistol_empire', (s) => bolt(s, { from: 2400, to: 330, len: 0.12, snap: 0.5, loud: 1.35, ring: 0.03, grit: 0.07 }));
  ...
  a.define('overheat', (s) => {
    s.tone({ wave: 'square', from: 660, to: 640, duration: 0.07, volume: 0.14, lowpass: 2000 });
    s.noise({ duration: 0.05, filter: 'highpass', from: 4000, to: 3000, volume: 0.18 });
  });
  a.define('blaster_overheat', (s) => {
    for (let i = 0; i < 3; i++) s.tone({ wave: 'square', from: 880, to: 860, duration: 0.07, delay: i * 0.11, volume: 0.15, lowpass: 2600 });
    s.noise({ duration: 0.5, filter: 'bandpass', from: 3000, to: 6000, q: 0.8, volume: 0.24 });
    s.tone({ wave: 'sawtooth', from: 180, to: 90, duration: 0.35, volume: 0.18, lowpass: 900 });
  });

  // ---- Bolts landing, burning, going by ----
  a.define('bolt_hit', (s) => {
    const p = s.pitch;
    s.noise({ duration: 0.06, filter: 'highpass', from: 6000 * p, to: 3000, volume: 0.6 });
    s.tone({ wave: 'square', from: 1500 * p, to: 240 * p, duration: 0.09, volume: 0.24, lowpass: 3500 });
    s.tone({ wave: 'sine', from: 320 * p, to: 90, duration: 0.1, volume: 0.35 });
    s.noise({ duration: 0.32, delay: 0.02, filter: 'bandpass', from: 2600 * p, to: 5200, q: 3, volume: 0.3 });
  });
  ...
  a.define('bolt_whizz', (s) => {
    const p = s.pitch;
    s.tone({ wave: 'sawtooth', from: 1500 * p, to: 380 * p, duration: 0.28, volume: 0.18, attack: 0.06, bandpass: { freq: 1400 * p, to: 500, q: 2 } });
    s.tone({ wave: 'sine', from: 900 * p, to: 260 * p, duration: 0.3, volume: 0.2, attack: 0.07 });
    s.noise({ duration: 0.22, filter: 'bandpass', from: 4200 * p, to: 1400, q: 1.5, volume: 0.24 });
  });

  // ---- The thermal detonator ----
  a.define('detonator_arm', (s) => {
    for (let i = 0; i < 3; i++) s.tone({ wave: 'square', from: 1500 + i * 300, to: 1500 + i * 300, duration: 0.045, delay: i * 0.09, volume: 0.14, lowpass: 3200 });
    s.tone({ wave: 'sine', from: 180, to: 420, duration: 0.3, delay: 0.27, volume: 0.15, attack: 0.1 });
  });
  a.define('toss', (s) => {
    s.noise({ duration: 0.2, filter: 'bandpass', from: 500, to: 1800, q: 1.2, volume: 0.8 });
  });
  a.define('clink', (s) => {
    const p = s.pitch;
    s.tone({ wave: 'triangle', from: 2350 * p, to: 2300 * p, duration: 0.16, volume: 0.24 });
    s.tone({ wave: 'sine', from: 3710 * p, to: 3650 * p, duration: 0.11, volume: 0.15 });
    s.noise({ duration: 0.03, filter: 'highpass', from: 5000, to: 4000, volume: 0.22 });
  });
  ...
  a.define('ui_hero_ready', (s) => {
    for (const [i, n] of [3, 10, 15, 22].entries()) s.tone({ wave: 'triangle', from: note(n), to: note(n), duration: 0.5, delay: i * 0.07, volume: 0.15 });
    s.tone({ wave: 'sine', from: note(27), to: note(27), duration: 0.9, delay: 0.28, volume: 0.08, vibrato: { rate: 7, depth: 12 } });
  });
  a.define('low_tickets', (s) => {
    for (let i = 0; i < 2; i++) {
      s.tone({ wave: 'square', from: 196, to: 185, duration: 0.3, delay: i * 0.38, volume: 0.15, lowpass: 900 });
      s.tone({ wave: 'sawtooth', from: 98, to: 96, duration: 0.3, delay: i * 0.38, volume: 0.12, lowpass: 500 });
    }
  });
  ...
  a.define('amb_wind', (s) => {
    ...
    for (let i = 0; i < 16; i++) {
      ...
      s.noise({ duration: 1.0, delay: i * 0.12, filter: 'bandpass', from: (240 + swell * 380) * p, to: (300 + swell * 300) * p, q: 0.9, volume: 0.05 + swell * 0.12 });
    }
  });
  a.define('amb_firefight', (s) => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      s.tone({ wave: 'sawtooth', from: f * (0.9 + Math.random() * 0.2), to: 260, duration: 0.12, delay: t, volume: 0.3, lowpass: 900 });
      s.noise({ duration: 0.05, delay: t, filter: 'lowpass', from: 900, to: 300, volume: 0.3 });
