...
export const ROWS: { y: number; ids: string[] }[] = [
  { y: 7.2, ids: ['imp_rifle', 'imp_heavy', 'imp_sniper', 'imp_pistol'] },
  { y: 4.6, ids: ['rebel_rifle', 'rebel_heavy', 'rebel_sniper', 'rebel_pistol'] },
...
function hall(): Blueprint {
  ...
  for (const z of [12, -12]) bp.fill({ x: 13, y: FLOOR, z }, { x: 13, y: FLOOR + 3, z }, 'gray_concrete');
  return bp;
}
...
export const shared = defineShared({
  ...meta,
  world: { terrain: 'void', structures: [hall()], spawn: { x: 13, y: FLOOR + 4, z: 12 }, spawnYaw: 0, time: 0.5, freezeTime: true },
