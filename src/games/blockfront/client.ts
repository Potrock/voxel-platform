...
/**
 * Blockfront II on each player's screen: the kits it uses, the platform's and its own (the standard
 ...
 * `client/bolts.ts`; detonators in the world; the spaceport's wind and distant fighting:
...
export default defineClient(shared, {
  kits: [...sounds.standard(), ...firstPerson.standard(), ...heroKits() /* the figures (heroes' sabers and powers too), the heroes' effects and HUD: heroes/client */, blasterHud(), hud.throwables(), vitals(), bolts(), effects.throwables(), ambience()],
  setup(client) {
    defineLooks(client);
    defineSounds(client);
  },
