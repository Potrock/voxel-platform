...
export default function cobskip() {
  ...
  // The controls: V on the keyboard, the D-pad's left on a controller, both on the home page.
  ...
  check(hints.some(([b, v]) => b === 'D-pad ←' && v === 'vote to skip'), `the controller's hint: ${JSON.stringify(hints)}`);
